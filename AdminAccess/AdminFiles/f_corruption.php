	...
	<div id="page-content" class="clearfix">
		...
		<div class="row-fluid">
			...
			<div class="table-header">
				Results for "Latest Repoted corruption"
			</div>
			...
			<table id="table_AD" class="table table-striped table-bordered table-hover">
				...
				<tbody>
					...
					$query = "SELECT * FROM s_corruption where  fwd_to='$role' AND Q_comment='' AND f_Ministry='$dep' order by Agenda_id desc ";
					$result = $localhost2->query($query);
					if ($result->num_rows > 0) {

						while($row = $result->fetch_assoc()) {
							$Agenda_fidcr = $row['Agenda_id'];
						...
						<?php 
							...
							echo "<a href=\"replycrf.php?Agenda_fidcr=$Agenda_fidcr\">";
							echo			'<button class="btn btn-mini btn-success"><i class="icon-ok">Reply</i></button>';
							echo "</a>";
							echo "<a href=\"forwardcrf.php?Agenda_fidcr=$Agenda_fidcr\">";
