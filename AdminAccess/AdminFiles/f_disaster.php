		...
		<div class="row-fluid">
			...
			<table id="table_AD" class="table table-striped table-bordered table-hover">
				...
				<tbody>
					...
					$query = "SELECT * FROM s_disaster where  fwd_to='$role' AND Q_comment='' AND f_Ministry='$dep' order by Agenda_id desc ";
					$result = $localhost2->query($query);
					if ($result->num_rows > 0) {

						while($row = $result->fetch_assoc()) {
							$Agenda_fidd = $row['Agenda_id'];
						...
						<?php 
							...
							echo "<a href=\"replydf.php?Agenda_fidd=$Agenda_fidd\">";
							echo			'<button class="btn btn-mini btn-success"><i class="icon-ok">Reply</i></button>';
							echo "</a>";
							echo "<a href=\"forwarddf.php?Agenda_fidd=$Agenda_fidd\">";
