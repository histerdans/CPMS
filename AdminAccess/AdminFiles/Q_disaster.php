	...
	<div id="page-content" class="clearfix">
		...
		<div class="row-fluid">
			...
			<div class="table-header">
				Results for "Latest Disastres Reported"
			</div>
			...
			<table id="table_QA" class="table table-striped table-bordered table-hover">
				...
				<tbody>
					...
					$query = "SELECT * FROM s_disaster order by Agenda_id desc";
					$result = $localhost2->query($query);
					if ($result->num_rows > 0) {

						while($row = $result->fetch_assoc()) {
							$Agenda_idd = $row['Agenda_id'];
