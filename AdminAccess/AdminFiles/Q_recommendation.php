	...
	<div id="page-content" class="clearfix">
		...
		<div class="row-fluid">
			...
			<div class="table-header">
				Results for "Latest Recommendation Passed"
			</div>
			...
			<table id="table_QA" class="table table-striped table-bordered table-hover">
				<thead>
					...
					<tbody>
						...
						$query = "SELECT * FROM s_recommendation order by Agenda_id desc";
						$result = $localhost2->query($query);
						if ($result->num_rows > 0) {

							while($row = $result->fetch_assoc()) {
								$Agenda_idr = $row['Agenda_id'];
